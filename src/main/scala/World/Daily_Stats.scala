...
object Daily_Stats {
  def main(args: Array[String]): Unit = {
    ...
    val topics = Array("world_daily_stats")
    ...
    // 预加载 date_dim 到内存 Map，key 格式 "year-month-day"
      ...
      .foreachRDD { rdd =>
        ...
        rdd.foreachPartition { partition =>
          ...
          val pstmt = conn.prepareStatement(
            """
              |INSERT INTO global_daily_stats (country_code, date_id, new_deaths, new_confirmed)
              |VALUES (?, ?, ?, ?)
          ...
          def safeToInt(s: String): Int = {
            ...
            else try s.trim.toInt catch { case _: NumberFormatException => 0 }
          }
          ...
          partition.foreach { fields =>
            try {
              ...
              val confirmed = safeToInt(fields(4))
              val deaths = safeToInt(fields(6))

              println(s"处理数据 - 日期: $dateStr, 国家代码: $countryCode, 新增确诊: $confirmed, 新增死亡: $deaths")
