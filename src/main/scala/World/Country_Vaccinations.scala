...
object Country_Vaccinations {
  def main(args: Array[String]): Unit = {
    ...
    val topics = Array("Country_Vaccinations")
    ...
    stream.map(_.value())
      ...
      .filter(fields =>
        fields.length >= 16 && fields(1).trim.length == 3 // ✅ 只保留国家代码长度为3的记录
      )
      .foreachRDD { rdd =>
        ...
        rdd.foreachPartition { partition =>
          ...
          val pstmt = conn.prepareStatement(
            """
              |INSERT INTO country_vaccinations (country_code, date_id, total_doses, vaccinated, fully_vaccinated, daily_doses, daily_vaccinated)
              |VALUES (?, ?, ?, ?, ?, ?, ?)
